...
export default function PatientAccountForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
      ...
      const response = await fetch('http://127.0.0.1:3030/pharmacy-0x2/api/pharmacy/update-info', {
        method: 'PUT',  
        headers: {
    ...
    <div>
      <h2 className={centerStyle}>Pharmacy General Information</h2>
            ...
            {pharmacyInfo.workingHours.map((day, index) => (
                <div key={index} className="w-full mx-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleWorkingHoursChange(index, 'openingTime', e.target.value)
                    }
                    disabled={!editMode}
                    required={editMode}
                  />
                  <input
                    ...
                    onChange={(e) =>
                      handleWorkingHoursChange(index, 'closingTime', e.target.value)
                    }
                    disabled={!editMode}
                    required={editMode}
                  />
          ...
          <div className="w-full flex-1 md:flex ">
                ...
                {editMode? <button className={submitButtonStyle} type="submit">
              Save Changes
            </button>:""}
                {userRole=="pharmacy manager"? <button
              type="button"
