...
export default function PopupInvenotryItemWindow({
  ...
  const handleDeleteItem = async (e) => {
    ...
    if (!confirmDeleteWindow) {
      ...
      await fetch(
        "http://127.0.0.1:3030/pharmacy-0x2/api/inventory/remove-item",
        {
          method: "DELETE",
          headers: {
  ...
  return (
    <div
      id="myModal"
      className="modal p-10px fixed w-full h-full top-0 left-0 flex items-center justify-center custom-z-pos-index"
    >
      <div className="modal-content border-2 border-gray-800 border-opacity-100 bg-white p-8 rounded shadow-lg relative">
        ...
        <h3 className="text-black my-5"> {popupWindowContent.name} </h3>{" "}
