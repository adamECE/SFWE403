...
export default function Reports() {
  ...
  for (let i = 0; i < quarterSets.length; i++) {
    if (
      currentDate.getMonth() > quarterSets[i][0] &&
  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:3030/pharmacy-0x2/api/reports/generateFinance", {
      ...
      .then((data) => {
        const tempTransactionData = data.filter((item) => {
          return item.status === "paid";
        });
        setTransactionData(tempTransactionData);

        let overCounterDataArray = [];
        let prescriptionDataArray = [];
        let overCounterObjArray = [];
        let prescriptionObjArray = [];
        tempTransactionData.forEach((transaction) => {
          ...
          overCounterDataArray.push(
            transaction.OverTheCounterItems.reduce(
              (n, { price }) => n + price,
              0
            )
          );
        ...
        for (let i = 0; i < tempTransactionData.length; i++) {
          ...
          const transactionDateStr =
            String(transactionDate.getMonth() + 1) +
            "/" +
            String(transactionDate.getFullYear());
          if (transactionDate >= startDate1 && transactionDate <= startDate2) {
            overCounterGraphDataArray[monthYearDict[transactionDateStr]] +=
              overCounterDataArray[i];
            prescriptionGraphDataArray[monthYearDict[transactionDateStr]] +=
              prescriptionDataArray[i];
            totalSalesForAverage[monthYearDict[transactionDateStr]] +=
              overCounterDataArray[i] + prescriptionDataArray[i];
          }
        }
  ...
  return (
    <div>
      ...
      {popupInit && (
        <div
          className="absolute bg-white border border-black bg-sky-500 rounded p-1 text-black"
          style={popupStyle}
        >
          $ {dataPopupValue}{" "}
        </div>
      )}{" "}
      {/* Select Date */} <h1 className="m-5 p-2"> Monthly Sales </h1>{" "}
      <div className="grid place-items-center">
        ...
        <div className="center w-full mx-5 my-1 p-2 text-white">
              ...
              onClick: (e, elements) => {
                ...
                if (elements.length > 0) {
                  if (elements[0].hasOwnProperty("element")) {
                    ...
                    setDataPopupValue(
                      Math.round(elements[0].element.$context.raw * 100) / 100
                    );
                    setPopupInit(true);
                  }
                }
        ...
        <div className="flex">
          ...
          <div
            className="center m-2 p-2 text-white border"
            style={{ backgroundColor: "#1478BD" }}
          ></div>{" "}
          <div className="center m-2 p-2 text-white"> Prescription Items </div>{" "}
          <div
            className="center m-2 p-2 text-white border"
            style={{ backgroundColor: "#ff721e" }}
          ></div>{" "}
        </div>{" "}
      </div>{" "}
    </div>
