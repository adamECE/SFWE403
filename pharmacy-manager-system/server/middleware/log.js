...
exports.logInventoryUpdate = asyncHandler(async (req, res, next) => {
  ...
  await log_invRemoval.save();
  if (req.invLogger.itemType == "removed")
    res.status(200).json({ message: "expired batch removed from inventory" });
  else return res.status(201).json({ message: "New Batch added to inventory" });
});

exports.logPurchase = asyncHandler(async (req, res, next) => {
  const logPurchase = new PurchaseLog({
    staffEmail: req.purchaseLogger.staffEmail,
    clientEmail: req.purchaseLogger.clientEmail,
    receiptID: req.purchaseLogger.receiptID,
    totalAmount: req.purchaseLogger.totalAmount,
  });
  await logPurchase.save();

  res.status(200).json({ message: req.purchaseLogger.receiptID });
});
